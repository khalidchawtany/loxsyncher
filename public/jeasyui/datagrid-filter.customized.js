(function($){
	...
	var extendedOptions = {
        ...
        field = (col.as)? col.as: field;  //if a field has as use it instead
				var input = $(this);
	...
	function init(target, filters){
		...
		opts.onBeforeLoad = function(param1, param2){
			if (param1){
				//if field has an as replace the sort with it
				var col = $(target).datagrid('getColumnOption', param1.sort);
		...
		function createFilter(frozen){
          ...
          //if a field has as use it instead
          var td;
          if(col && col.as){
              td = $('<td></td>').attr('field', field).attr('as',col.as).appendTo(tr);
          } else{
