                    ...
                    <table class="has_border" style="width: auto; ">
                        <tr>
                            ...
                            <td style="min-width: 100px; direction:ltr;">
                                {{ number_format($info['invoices_amount']) }} IQD </td>
                            <th style="width: 130px;"> پارەی وەرگیراو:</th>
                            <td style="min-width: 100px; direction:ltr;">
                                {{ number_format($info['received_invoices_amount']) }} IQD </td>
                        </tr>
                        <tr>
                            <th>عددی پسوڵە:</th>
                            <td>{{ $info['invoices_count'] }}</td>
                            <th>پسوڵەی وەرگیراو:</th>
        ...
        <table class="has_border is_minimal" style="width:100%;">
            ...
            @foreach ($invoices as $invoice)
                <tr>
                    ...
                    <td>{{ \Carbon\Carbon::parse($invoice->payment_date)->format('h:i:sA d/m/Y') }}</td>
                    <td>{{ $invoice->office_name }}</td>
                    <td>{{ $invoice->received_by_name }}</td>
                    <td>{{ \Carbon\Carbon::parse($invoice->at)->format('d/m/Y') }}</td>
                </tr>
            @endforeach
        </table>
    </section>
