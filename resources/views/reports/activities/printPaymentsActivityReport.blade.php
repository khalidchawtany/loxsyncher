@php
  //da($rows->first());
    $from = null;
    $to = null;

    $filterRules = collect(json_decode(request('filterRules')));

    $filterRules->each( function($filterRule) use(&$from, &$to) {

        if($filterRule->field == 'payments.created_at'
            && isset($filterRule->value)) {

            $value = $filterRule->value;
            $from = $value;

            if (str_contains($value, ',')) {
                $from = str_before($value, ',');
                $to   = str_after($value, ',');
            }
        }
    });
@endphp
@extends('layouts.print')
@push('styles')

    @page { size: A4 }

    body{
        padding: 0px; margin:0px; }

    table {
        border-collapse: collapse;
        width: 100%;
    }

    th{
        solid;background-color:#ddd
    }

    section.sheet {
        page-break-inside: initial;
    }

    tr {
        page-break-inside: avoid;
    }

    table.has_border, table.has_border th, table.has_border td {
        border: 1px solid grey;
        padding: 0px 5px;
        text-align:center;
    }
    table.has_border.is_minimal , table.has_border.is_minimal th, table.has_border.is_minimal td {
        padding: 0px 0px;
        font-size: 11px;
        line-height: 12.5px;
        width: 100%;
    }
@endpush
@include('styles')
@section('body-attributes') class="A4" style="font-family:Droid Arabic Naskh; font-size: 14px;" @endsection
@section('content')
    <section class="sheet" style="padding:15px;">
        <table style="width: 100%;">
            <tr>
                <td style="text-align: right; width: 100px;">
                    <table class="has_border">
                        <tr>
                            <th style="width: 100px;">لە: </th>
                            <td style="min-width: 100px;"> {{ $from }} </td>
                        </tr>
                        <tr>
                            <th>بۆ:</th>
                            <td>{{ $to }}</td>
                        </tr>
                    </table>
                </td>

                <td>
                    <h1 style="text-align:center; direction:ltr;">
                      Payment Changes Report
                    </h1>
                </td>


            </tr>
        </table>
        <table class="has_border small ltr" style="width:100%;">
            <tr>
                <th>Id</th>
                <th>Date</th>
                <th>Update Count</th>
                <th colspan="2">Paid Amount</th>
                <th>User</th>
            </tr>
            @foreach ($rows as $row)
                <tr>
                    <td>{{ $row->id }}</td>
                    <td>{{ $row->date }}</td>
                    <td>{{ $row->activity_count }}</td>
                    <td colspan="2">{{ $row->paid_amount }}</td>
                    <td>{{ $row->user_name }}</td>
                </tr>
                @if($row->activities)
                  <tr>
                    <th colspan="3">Old Values</th>
                    <th colspan="3">New Values</th>
                  </tr>
                @endif
                @foreach($row->activities as $activity)
                  @php
                    $props = json_decode($activity->properties);
                    $old = optional($props)->old;
                    $new = optional($props)->attributes;
                  @endphp

                  <tr>
                    <td colspan="3">
                      User: {{ $activity->user_name }}
                    </td>
                    <td colspan="3">
                      Date/Time: {{ $activity->date }}
                    </td>
                  </tr>
                  <tr>
                    <td  class="w-50p" style="padding:0px;" colspan="3">
                      @if($old != null)
                      <table class="small">
                        @foreach($old as $key => $field)
                        <tr>
                          <td class="w-50p"> {{ $key }}: </td>
                          <td> {{ $field }} </td>
                        </tr>
                        @endforeach
                      </table>
                    @endif
                    </td>
                    <td  class="w-50p" style="padding:0px;" colspan="3">
                      <table class="small">
                        @foreach($new as $key => $field)
                        <tr>
                          <td class="w-50p"> {{ $key }}: </td>
                          <td> {{ $field }} </td>
                        </tr>
                        @endforeach
                      </table>
                    </td>
                  </tr>
                @endforeach
            @endforeach
        </table>
    </section>
@endsection
