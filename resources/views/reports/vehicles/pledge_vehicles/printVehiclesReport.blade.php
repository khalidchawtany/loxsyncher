...
@section('content')
    ...
    <section class="sheet" style="padding:15px;">
        ...
        <table class="has_border" style="width:100%; direction: ltr;">
            ...
            @foreach ($vehiclesSums as $vehiclesSum)
                <tr>
                    {!! $departmentName == null ? '<td>' . $vehiclesSum->department_name . '</td>': '' !!}
                    <td>{{ $vehiclesSum->product_name}}</td>
                    <td>{{ number_format($vehiclesSum->product_count) }}</td>
                    <td>{{ number_format($vehiclesSum->amount_sum,2) }}</td>
                    <td>{{ $vehiclesSum->amount_unit }}</td>
                </tr>
            @endforeach
        </table>
    </section>
